...
/**
 * This constraint enforces m=MIN_i x_i.
 *
 ...
 * @author Yan Georget
 */
public class Min extends BaseConstraint {
    //------------------------------------------------------------------------
    // PROPERTIES
    //------------------------------------------------------------------------
     ...
     */    
    public Min(IntegerVariable m, IntegerVariable[] x) {
        super();
        this.name = "m=MIN_i x_i";
        this.m = m;
    ...
    public void filter(ChoicePointStack cp,
        ...
        for (int j=variables.length; --j>0;) {
            ...
            x.adjustMin(cp, cs, this, min);
            m.adjustMax(cp, cs, this, x.getMax());
        }
        final int max = mDom.getMax();
        int nbEqual = 0; // nb possibly equal
        int jEqual = -1; // last possibly equal
        for (int j=variables.length; --j>0;) {
            if (max >= ((IntegerVariable) variables[j]).getMin()) {
                nbEqual++;
                jEqual = j;
            }
        }
